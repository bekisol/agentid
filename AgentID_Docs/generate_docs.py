...
CHANGELOG = [
    ("v2.1.1", "2026-05-18",
     "fix(audit-followup): two MED issues from post-commit hostile review. "
     "evidence_bundle: participant_snapshots now pinned to run start time — "
     "SQL gains 'AND snapshot_date <= run.created_at' so auditors see the trust state "
     "the orchestrator actually observed, not today's scores; each entry tagged "
     "snapshot_source='at_run_time'. "
     "dimension_policy: dedup DELETE (keep MAX(id) per did/policy_id where lifted_at IS NULL) "
     "runs before CREATE UNIQUE INDEX uq_acr_active, making the migration safe on databases "
     "with pre-existing duplicate active restrictions from the old SELECT-then-INSERT race. "
     "167/167 tests passing. Commit 70c5370."),
    ("v2.1.0", "2026-05-18",
     "fix(audit): all 9 AUDIT_PHASE_1.md findings resolved. "
     "HIGH: breach demotion persistence — _check_sla_breach consolidated to single transaction; "
     "_recompute_sla_score subtracts cumulative contract_breach_penalties (30d window) from rolling "
     "average, making demotion durable across future recomputes; take_snapshot_for_did called "
     "synchronously post-commit instead of daemon thread. "
     "HIGH: 5 silent except:pass blocks replaced with logger.error calls. "
     "MED: evidence pack endpoint — GET /pro/groups/runs/{run_id}/evidence-pack (run-scoped, "
     "includes causal graph + participant snapshots + Merkle root + platform HMAC-SHA256 sig); "
     "POST /pro/evidence-pack/verify for independent verification. "
     "MED: per-node trust snapshots — orchestrator_rejected events now emit dimension_scores; "
     "misleading 'latest' enrichment in causal_graph.py removed; null is honest for auditors. "
     "MED: restriction upsert race closed — uq_acr_active partial unique index + INSERT ON "
     "CONFLICT DO UPDATE + atomic hysteresis increment via UPDATE RETURNING. "
     "MED: _check_contract_gate expiry path reads breach_consequence.dimension/demotion_magnitude "
     "from contract (was hardcoded d6/0.1). "
     "MED: 5 separate _get_conn transactions in _check_sla_breach consolidated to 1. "
     "MED: nightly suspension sweeper (sweep_overdue_suspensions) catches contracts at threshold "
     "with no further calls; wired into server_pro.py nightly batch. "
     "LOW: _filter_eligible_candidates now pre-filters dimension-restricted agents before LLM plan; "
     "get_restricted_dids(owner) added to dimension_policy.py. "
     "167/167 tests passing (18 new tests). "
     "Files: capability_contracts.py, dimension_policy.py, causal_graph.py, evidence_bundle.py, "
     "group_orchestrator.py, server_pro.py, tests/test_contract_enforcement.py, "
     "tests/test_causal_graph.py."),
    ("v2.0.3", "2026-05-18",
     "fix(1.3): eliminate multi-version ambiguity in SLA breach pipeline. "
     "capability_contracts allows multiple active versions of the same (did, capability) pair "
     "(re-registering only deactivates matching version). The breach pipeline was picking an "
     "indeterminate contract via LIMIT 1 without ordering, and _suspend_contract was updating "
     "ALL active versions. "
     "Both active-contract SELECTs in _check_sla_breach now use ORDER BY id DESC so the "
     "newest active contract is always chosen deterministically. "
     "_suspend_contract now takes contract_id as first arg and uses WHERE id = %s — only the "
     "specific contract that accumulated the breaches is suspended; other active versions "
     "(different version numbers) are unaffected. "
     "Note: pre-Phase-1.3 sla_breach_events rows with contract_id IS NULL are intentionally "
     "invisible to the new contract-scoped queries — treated as a clean reset for those rows "
     "since their contract association is unknown. "
     "162/162 tests passing. "
     "Files: agentid-pro/capability_contracts.py, "
     "agentid-pro/tests/test_contract_enforcement.py."),
    ("v2.0.2", "2026-05-18",
     "fix(1.3): scope sla_breach_events queries to contract_id. "
     "Debounce check, 30-day breach count, and enforcement-history endpoint were querying "
     "sla_breach_events by (did, capability) only. A re-registered contract with the same "
     "DID/capability pair would inherit the old contract's 24h debounce window, inflated "
     "breach counts, and unrelated history rows. "
     "All three queries now include AND contract_id = %s. "
     "162/162 tests passing. "
     "Files: agentid-pro/capability_contracts.py."),
    ("v2.0.1", "2026-05-18",
     "Phase 1.3 — round-2 bug fixes; 162/162 tests passing. "
     "[Critical] _recompute_sla_score CTE selected only latency_ms but FILTER "
     "referenced r.success — Postgres would error at runtime, exception swallowed, "
     "SLA score always None so _check_sla_breach never fired. Fixed: added success "
     "to CTE SELECT. "
     "[Critical] _check_sla_breach initial SELECT added id as first column (needed "
     "to populate sla_breach_events.contract_id). Row unpacking changed from "
     "(target, remedies, owner) to (id, target, remedies, owner); null check updated "
     "to row[1] is None. "
     "[High] Async dispatch path called _log_call(latency_ms=0, sla_met=None) at "
     "dispatch time, treating every async invocation as a successful SLA call before "
     "the task even started. Removed dispatch-time _log_call entirely; only "
     "on_task_completed() logs, with real elapsed_ms computed from invoked_at. "
     "[Medium] Expiry D6 demotion ran in a background thread (fire-and-forget). "
     "Changed to synchronous inline call so demotion is guaranteed before the 402 "
     "response. "
     "3 test mock tuples updated from 3-element to 4-element after SELECT id added. "
     "Files: agentid-pro/capability_contracts.py, agentid-pro/invocation.py, "
     "agentid-pro/tests/test_contract_enforcement.py."),
    ("v2.0.0", "2026-05-18",
    ...
    ("v1.9.8", "2026-05-17",
     ...
     "_load_capability_profile(), so capability-specific prompt injection and "
     "permission enforcement were silently skipped for approved runs. Fixed: after "
     "unpacking each pending row, _load_capability_profile(_cap, owner, group_id) is "
     "now called; the resolved permission_level from the profile overrides the stored "
     "value; the assignment row UPDATE now writes required_capability and "
     "resolved_permission_level (matching normal dispatch); the agent_assigned event "
     "now includes profile_resolved=True/False so the audit graph records whether the "
     "profile lookup succeeded; _execute_subtask is started with kwargs={'profile': "
     "_ap_profile} matching the kwargs=... pattern in normal dispatch. "
     "[Medium] The fallback denied UPDATE in normal dispatch (_orchestrate) used "
     "UPDATE ... WHERE ... LIMIT 1 which is invalid PostgreSQL syntax. Fixed by "
     "replacing with WHERE id = (SELECT id FROM group_run_assignments WHERE "
     "group_run_id=%s AND agent_did=%s AND task_id IS NULL ORDER BY assigned_at LIMIT 1), "
     "matching the allow-path fallback pattern already in place. "
     "The denied path in approval dispatch also updated to include required_capability "
     "and resolved_permission_level for schema parity. "
     "Three new regression tests added: "
     "test_dispatch_pending_logs_profile_resolved_in_agent_assigned — verifies the "
     "agent_assigned event emitted by approval dispatch includes profile_resolved=True "
     "and permission_level from the resolved profile object. "
     "test_dispatch_pending_profile_kwarg_passed_to_execute_subtask — verifies that "
     "_execute_subtask is called with profile=<resolved profile object> as a kwarg. "
     "test_normal_dispatch_denied_fallback_uses_subquery_not_limit — source inspection "
     "test verifying the old bare LIMIT 1 pattern is absent and the subquery pattern "
     "is present in group_orchestrator.py. "
     "Files: agentid-pro/group_orchestrator.py, "
     "agentid-pro/tests/test_group_orchestrator_xalgo.py."),
    ("v1.9.7", "2026-05-17",
     "Phase 1.2 — Causal Audit Graph round-7 review fixes; 146/146 tests passing. "
     "[High] Normal dispatch updated assignment rows by agent_did, not by specific "
     "assignment: if the same agent appeared twice in a plan, a policy denial or the "
     "allow-path UPDATE could affect all pending rows for that agent. Fixed by adding "
     "RETURNING id to the plan assignment INSERT, building a _plan_assign_ids dict "
     "(plan index → assignment row id) in the same atomic transaction, then using "
     "WHERE id = %s in both the policy-denied UPDATE and the allow-path UPDATE. "
     "Fallback to the old agent_did filter is retained when _assign_id is None "
     "(defensive; RETURNING id should never fail in practice). "
     "FakeCursor updated: plan INSERT (detected by 'candidate_score' in query text) "
     "returns a synthetic integer id so _plan_assign_ids is populated in tests, while "
     "retry INSERT is still detected by the absence of 'candidate_score'. "
     "[Medium] No approval-dispatch tests: added two: "
     "test_dispatch_pending_passes_persisted_capability_to_policy_gate — verifies that "
     "_dispatch_pending_assignments reads required_capability from the assignment row "
     "('payments') and passes it to _run_policy_gate, not the fallback 'general'. "
     "test_dispatch_pending_all_policy_denied_triggers_synthesis — verifies that when "
     "_run_policy_gate denies all pending assignments, _synthesize_run is spawned after "
     "the loop. Uses subclassed FakeCursor returning a single pending row and the same "
     "synchronous threading shim pattern. "
     "Files: agentid-pro/group_orchestrator.py, "
     "agentid-pro/tests/test_group_orchestrator_xalgo.py."),
    ("v1.9.6", "2026-05-17",
     "Phase 1.2 — Causal Audit Graph round-6: regression test coverage for the two "
     "prior round-5 fixes; 144/144 tests passing. "
     "test_orchestrate_persists_required_capability_in_assignment_row: verifies that "
     "the plan INSERT writes required_capability=item.get('capability') into "
     "group_run_assignments, so approval dispatch reads the real planned capability "
     "rather than falling back to 'general' for policy gate checks. Uses "
     "require_plan_approval=True to isolate the INSERT without mocking dispatch machinery. "
     "test_orchestrate_all_policy_denied_triggers_synthesis: verifies that when "
     "_run_policy_gate denies all assignments, the post-loop _all_assignments_done check "
     "spawns _synthesize_run. Uses a synchronous threading.Thread shim on the freshly "
     "loaded module (go.threading = fake_threading) for deterministic, race-free assertion. "
     "Files: agentid-pro/tests/test_group_orchestrator_xalgo.py."),
    ("v1.9.5", "2026-05-17",
     "Phase 1.2 — Causal Audit Graph round-5 review fixes; 142/142 tests passing. "
     "[Critical] Approval dispatch capability policy bypass: assignment rows created at "
     "plan time had no required_capability column set, so _dispatch_pending_assignments() "
     "read NULL and fell back to 'general' when calling _run_policy_gate(). Any restriction "
     "or policy scoped to the actual capability (e.g. 'research', 'payments') was silently "
     "missed after approval. Fixed: the assignment INSERT now includes required_capability = "
     "item.get('capability', 'general') so approval dispatch reads the real capability. "
     "[High] Policy-denied dispatch can leave run stuck in 'running': both normal and "
     "approval dispatch mark denied assignments as 'failed' and continue, but neither path "
     "called _all_assignments_done() / _synthesize_run() after the loop. If all assignments "
     "were denied before any thread started, no on_agent_task_completed hook fired and the "
     "run sat in 'running' indefinitely (watchdog does not force-fail runs with zero "
     "non-terminal assignments since _all_assignments_done() returns True immediately). "
     "Fixed by adding an _all_assignments_done() check after the dispatch loop in both "
     "_orchestrate() and _dispatch_pending_assignments(). If True, spawns a synthesis "
     "thread. _synthesize_run() has its own re-entrancy guard "
     "(status in completed/failed/synthesizing/cancelled → early return) so a race with "
     "a fast worker thread is harmless. "
     "Files: agentid-pro/group_orchestrator.py."),
    ("v1.9.4", "2026-05-17",
     "Phase 1.2 — Causal Audit Graph round-4 review fixes; 142/142 tests passing. "
     "[Critical] Approval-required runs bypassed the control-plane policy gate: "
     "_dispatch_pending_assignments() started tasks without calling _run_policy_gate(), "
     "so active capability restrictions, budget checks, review requirements, and the "
     "Phase 1.1 dimension restriction enforcement loop could all be bypassed by an "
     "approved run. Fixed by adding the same _run_policy_gate() call pattern as normal "
     "dispatch, positioned BEFORE the UPDATE to 'running' so the gate runs against the "
     "latest restriction state at actual execution time, not at plan time. "
     "On denial: logs policy_denied (parented to approval_granted), marks assignment "
     "failed, continues to next — matching normal dispatch behavior exactly. "
     "[Medium] policy_denied events unparented in normal dispatch: _log_event call at "
     "line 2136 had no parent_event_id. Fixed to pass parent_event_id=_plan_event_id "
     "so denial traces prove which plan caused the blocked dispatch. "
     "Approval-path policy_denied also parented to approval_granted. "
     "[Low] New approval and correction edges fell back to 'sequence' label: "
     "_EDGE_TYPE_MAP extended with 13 new entries covering the full correction and "
     "approval chains: "
     "run_created → orchestrator_plan_corrected = 'correction'; "
     "orchestrator_plan_corrected → orchestrator_plan_dim_policy_corrected = 'policy_correction'; "
     "orchestrator_plan_corrected → orchestrator_plan = 'planning'; "
     "orchestrator_plan → approval_requested = 'approval_pending'; "
     "approval_requested → approval_granted = 'approval'; "
     "approval_granted → agent_assigned = 'dispatch'. "
     "Module docstring updated to document all edge types. "
     "10 new edge classification tests added. "
     "Files: agentid-pro/causal_graph.py, agentid-pro/group_orchestrator.py, "
     "agentid-pro/tests/test_causal_graph.py."),
    ("v1.9.3", "2026-05-17",
     "Phase 1.2 — Causal Audit Graph round-3 review fixes; 132/132 tests passing. "
     "[High] Policy correction events detached from plan chain: "
     "orchestrator_plan_corrected, orchestrator_plan_dim_policy_corrected, and "
     "orchestrator_plan_dim_policy_all_blocked were logged with no parent. "
     "Fixed by moving the run_created id lookup to before the correction block, "
     "introducing _last_corr_event_id to thread the latest correction id forward, "
     "and making each correction event parent to the previous one. "
     "orchestrator_plan now parents to _last_corr_event_id or run_created, so the "
     "full chain is: run_created -> (corrections) -> orchestrator_plan. "
     "The redundant run_created lookup that was immediately before orchestrator_plan "
     "is removed since _run_created_eid is now set earlier. "
     "[High] Approval chain not causally auditable: "
     "approval_requested was a root, approval_granted was a root, and "
     "approval-path agent_assigned parented directly to orchestrator_plan, "
     "making it impossible to prove in a trace that approval was granted before dispatch. "
     "Fixed: approval_requested parents to _plan_event_id; approval_granted parents to "
     "approval_requested (via lookup); agent_assigned in approval path parents to "
     "approval_granted. _dispatch_pending_assignments gains an optional "
     "approval_granted_eid param (passed by approve_group_run_plan via Thread args) "
     "with a fallback DB lookup for backward compatibility. "
     "run_failed (no AI on approval) also parented to approval_granted. "
     "Full approval chain: orchestrator_plan -> approval_requested -> approval_granted "
     "-> agent_assigned. "
     "[Medium] Approval-path snapshot parity: normal dispatch records trust_score, "
     "trust_level, capability, permission_level, and dimension_scores; approval dispatch "
     "only recorded dimension_scores. Fixed: _dispatch_pending_assignments now SELECTs "
     "required_capability and resolved_permission_level from the assignment row and calls "
     "_fetch_trust_info(agent_did) to get trust_score and trust_level. "
     "[Low] _log_event parent validation used a new connection even when the caller "
     "passed conn=. If parent and child are inserted in the same uncommitted transaction, "
     "the validator would not see the parent and drop the link. Fixed: when conn is "
     "provided, the validation query runs on that same connection; a new connection is "
     "only opened when no conn is passed (the existing behavior for standalone inserts). "
     "Files: agentid-pro/group_orchestrator.py."),
    ("v1.9.2", "2026-05-17",
     "Phase 1.2 — Causal Audit Graph round-2 review fixes; 132/132 tests passing. "
     "[High] synthesis_started parent silently dropped when all agent_completed events lack "
     "causal_weight: the parent query filtered WHERE causal_weight IS NOT NULL, so runs "
     "where quality_score was None (quality evaluator failure, low-confidence rejection) "
     "produced an unlinked synthesis_started. Fixed to ORDER BY causal_weight DESC NULLS LAST "
     "so any agent_completed is selected as parent when weighted ones are unavailable. "
     "[High] run_failed at synthesis-no-AI-config not linked to synthesis_started: "
     "_synth_started_id was in scope but not passed as parent_event_id. Fixed. "
     "[High] run_failed in _orchestrate() outer except not linked to plan/root: "
     "_plan_event_id may not exist if exception fires before orchestrator_plan event is logged "
     "(early LLM failure). Fixed to lookup _lookup_latest_event_id(run_id, 'orchestrator_plan') "
     "or 'run_created' — whichever is latest — so the failure is always chained to "
     "something meaningful regardless of where in the orchestration the exception occurred. "
     "[High] run_failed at _orchestrate() no-AI-config not linked to run_created: "
     "fires before _plan_event_id exists; fixed to lookup 'run_created' via helper. "
     "[Medium] agent_assigned error path missing parent link: the dispatch catch block at "
     "line 2189 logged agent_assigned without parent_event_id even though _plan_event_id is "
     "in scope. Fixed to pass parent_event_id=_plan_event_id consistently with normal path. "
     "[Medium] trace_event_ancestors() missing batch trust enrichment: get_run_causal_graph() "
     "enriches trust snapshots at read time but trace_event_ancestors() only called "
     "_extract_trust_snapshot(), returning null for rejection/synthesis nodes. Added the same "
     "batch enrichment block (with snapshot_source='latest') to trace_event_ancestors() so "
     "both endpoints return consistent trust data. "
     "[Low] Orphan sweeper run_failed not linked to any parent: fixed to lookup "
     "'orchestrator_plan' or 'run_created' for the orphaned run id before logging. "
     "New tests: test_trace_batch_enrichment_fills_missing_snapshot, "
     "test_trace_batch_enrichment_error_does_not_raise. "
     "Files: agentid-pro/causal_graph.py, agentid-pro/group_orchestrator.py, "
     "agentid-pro/tests/test_causal_graph.py."),
    ("v1.9.1", "2026-05-17",
     "Phase 1.2 — Causal Audit Graph round-1 review fixes; 130/130 tests passing. "
     "[Critical] CTE cross-run boundary: trace_event_ancestors() recursive CTE now adds "
     "AND e.group_run_id = %s to the recursive term (run_id passed as third parameter). "
     "Previously only the anchor was constrained, so a stale parent_event_id pointing at "
     "another run's event would cause traversal to escape into that run's event stream. "
     "Defense-in-depth: _log_event() validates that the supplied parent_event_id belongs to "
     "the same group_run_id before writing; drops the link (fail-open) if validation fails. "
     "[High] Approval-gated dispatch missing causal links: _dispatch_pending_assignments() "
     "approval path now calls _lookup_latest_event_id(run_id, 'orchestrator_plan') to find "
     "the plan event and passes parent_event_id + dimension_scores in each agent_assigned "
     "event, matching the normal dispatch path. "
     "[Medium] Trust snapshots only on agent_assigned (write time): agent_completed events "
     "now also carry dimension_scores in event_data (fetched via _fetch_trust_info at "
     "completion time) so the trust state at output acceptance is preserved in the graph. "
     "Additionally, get_run_causal_graph() batch-enriches remaining agent-scoped nodes "
     "(rejection/synthesis events) at read time via batch_fetch_dimension_scores() with "
     "snapshot_source='latest' marker so callers can distinguish exact-at-event vs approximate. "
     "Enrichment errors are caught and silently skipped (graph is still returned). "
     "[Medium] Four failure events wired to parents: execution error orchestrator_rejected "
     "links to agent_assigned; watchdog-timeout orchestrator_rejected links to "
     "orchestrator_plan; plan parse failure run_failed links to run_created; synthesis "
     "failure run_failed links to synthesis_started. "
     "New tests in test_causal_graph.py: test_no_trust_data_and_no_agent_did_snapshot_stays_none, "
     "test_batch_enrichment_error_does_not_raise, test_cross_run_isolation_stops_at_run_boundary. "
     "Files: agentid-pro/causal_graph.py, agentid-pro/group_orchestrator.py, "
     "agentid-pro/tests/test_causal_graph.py."),
    ("v1.9.0", "2026-05-17",
     "Phase 1.2 — Causal Audit Graph. "
     "NEW MODULE causal_graph.py: two read-only endpoints exposing group_run_events "
     "parent_event_id links as a queryable DAG. "
     "Schema: added parent_event_id BIGINT FK (self-referential) and causal_weight FLOAT "
     "to group_run_events via CREATE TABLE + idempotent ALTER TABLE migrations; "
     "added idx_gre_parent index. "
     "_log_event() updated: new keyword-only params parent_event_id and causal_weight; "
     "INSERT now uses RETURNING id so the function returns the inserted event id (Optional[int]). "
     "Fully backward-compatible — existing call sites pass no keyword args. "
     "New helper _lookup_latest_event_id(run_id, event_type, agent_did=None) for resolving "
     "parent event ids at write time without threading state through function signatures. "
     "Causal edges wired at 7 lifecycle points: "
     "orchestrator_plan → parent=run_created (DB lookup); "
     "agent_assigned → parent=orchestrator_plan + dimension_scores added to event_data for "
     "trust snapshot on graph node; "
     "agent_completed → parent=agent_assigned, causal_weight=quality_score; "
     "orchestrator_rejected → parent=last agent_completed or agent_assigned; "
     "synthesis_started → parent=highest-causal_weight agent_completed; "
     "synthesis_completed → parent=synthesis_started; "
     "run_completed → parent=synthesis_completed. "
     "API endpoints: GET /pro/groups/runs/{run_id}/causal-graph returns "
     "{nodes, edges, node_count, edge_count} — each node has trust_snapshot (extracted "
     "from event_data when present), each edge has edge_type derived from parent/child "
     "event_type pair (planning/dispatch/execution/rejection/quality_fail/contribution/"
     "finalization/completion/failure/sequence). "
     "GET /pro/groups/runs/{run_id}/causal-graph/trace?event_id=X uses a recursive CTE "
     "to walk the ancestor chain back to the root, returning events chronologically with "
     "depth_from_target field. "
     "Pre-Phase-1.2 runs (no parent links) return full event list with empty edges. "
     "Tests: tests/test_causal_graph.py — 27 tests covering edge type classification, "
     "trust snapshot extraction, graph building (empty/root/chain/weight/orphan), and "
     "trace ancestor chain. "
     "Fixed: test_group_orchestrator_xalgo.py _log_event stub updated to accept **kw "
     "(new keyword args). Full suite: 128/128 passed. "
     "Files: agentid-pro/causal_graph.py (new), agentid-pro/group_orchestrator.py, "
     "agentid-pro/server_pro.py, agentid-pro/tests/test_causal_graph.py (new), "
     "agentid-pro/tests/test_group_orchestrator_xalgo.py."),
    ("v1.8.4", "2026-05-17",
     "Phase 1.1 review fixes (round 4) — blocked assignment lifecycle fully closed; "
     "101/101 tests passing. "
     "[High] 'denied' added to terminal status filter in all three locations: "
     "_get_pending_assignments() WHERE clause (line 843), watchdog force-fail UPDATE "
     "WHERE clause (line 1251), and _all_assignments_done() COUNT WHERE clause (line 1275). "
     "Previously 'denied' rows counted as active, so partial-block runs would never "
     "synthesize after their live assignments finished. Now all three gates treat 'denied' "
     "as terminal alongside 'completed', 'failed', 'reassigned'. "
     "[Medium] Partial-block denied rows now preserve subtask: the partial-block INSERT "
     "(inside the main assignment transaction) was still writing empty string for subtask "
     "despite the correction entry carrying blk.get('subtask', ''). Fixed to use "
     "blk.get('subtask', '') consistently with the all-block path. "
     "[Low] All-block failed run sets completed_at: the UPDATE group_runs in the all-block "
     "fast-path now writes completed_at = NOW() alongside status = 'failed' and "
     "updated_at = NOW(), consistent with how other terminal run states are recorded. "
     "File: agentid-pro/group_orchestrator.py."),
    ("v1.8.3", "2026-05-17",
     "Phase 1.1 review fixes (round 3) — blocked assignment lifecycle fully wired; "
     "101/101 tests passing. "
     "[High] Durable assignment records for blocked items: "
     "enforce_plan_dimension_policy_gate() now includes subtask + capability in each "
     "blocked correction entry. group_orchestrator.start_group_run() handles two cases: "
     "(1) ALL items blocked — run is immediately set to 'failed', a distinct "
     "orchestrator_plan_dim_policy_all_blocked event is logged with blocked count + "
     "violation reasons, denied assignment rows (status='denied', rejection_reason=policy "
     "violation message, completed_at=NOW()) are written for every blocked item, and the "
     "function returns early so no 'running' state is ever set for a zero-assignment run. "
     "(2) PARTIAL block — run proceeds as 'running' with the reduced plan, and denied "
     "assignment rows are inserted in the same DB transaction as the active assignment rows, "
     "so synthesis and audit can account for all originally planned subtasks. "
     "[Low] Stale audit message fixed: orchestrator_plan_dim_policy_corrected event now "
     "logs replaced_count and blocked_count separately and builds an accurate note string "
     "describing both outcomes (replaced N / blocked M) rather than always saying 'replaced'. "
     "Files: agentid-pro/dimension_policy.py, agentid-pro/group_orchestrator.py, "
     "agentid-pro/tests/test_dimension_policy.py."),
    ("v1.8.2", "2026-05-17",
     "Phase 1.1 review fixes (round 2) — 5 findings resolved; 40/40 dimension policy tests, "
     "101/101 full suite. "
     "[Critical] Fallback bypass removed: when no compliant capable replacement exists, "
     "enforce_plan_dimension_policy_gate() now blocks the assignment entirely (drops item "
     "from corrected_plan, correction entry has blocked=True, replaced_with=None) instead "
     "of routing to highest-trust violating agent. This is the correct enforcement behavior "
     "for a policy engine — an unroutable assignment is preferable to a policy violation. "
     "[High] D6 dispatch enforcement wired: batch_fetch_dimension_scores() now selects d6 "
     "and returns it in the score dict, so dispatch policies on capability_trust evaluate "
     "against the real stored value instead of always seeing None. "
     "[High] D6 snapshot TypeError fixed: trust_score.py emits capability_trust as "
     "{cap: {\"score\": float, \"signals\": ..., \"detail\": ...}} not plain floats. "
     "dimension_snapshots.py now extracts v.get(\"score\") from dict values before float(), "
     "with graceful fallback for plain-float form (older data). "
     "[Medium] _run_policy_gate() dimension restriction check moved before the policy/budget "
     "ImportError block in group_orchestrator.py — the restriction check now runs first and "
     "cannot be skipped by an ImportError in an unrelated module. "
     "[Medium] GET /pro/dimension-policies now returns policy_set_version at both envelope "
     "level (active_version) and per-policy (policy_set_version field), so operators can "
     "confirm which version is live without a separate API call. "
     "Test updated: test_all_agents_violated_fallback_to_highest_trust renamed to "
     "test_all_agents_violated_blocks_assignment with corrected assertions. "
     "Files: agentid-pro/dimension_policy.py, agentid-pro/dimension_snapshots.py, "
     "agentid-pro/group_orchestrator.py, agentid-pro/tests/test_dimension_policy.py."),
    ("v1.8.1", "2026-05-17",
     "Phase 1.1 review fixes — 5 correctness issues addressed; 40/40 tests still passing. "
     "[Critical] Closed enforcement loop: has_active_restriction() now wired into "
     "_run_policy_gate() in group_orchestrator.py — the first gate hit at subtask execution. "
     "Agents restricted by apply_score_update_enforcement() are now blocked from executing "
     "subtasks (previously score-update enforcement wrote restrictions but nothing read them "
     "at runtime). "
     "[High] D6 (capability_trust) added to dimension_snapshots: d6 FLOAT column added to "
     "agent_dimension_snapshots table (CREATE TABLE + ALTER TABLE IF NOT EXISTS migration), "
     "take_snapshot_for_did() now computes d6 as min(capability_trust.values()) for the most "
     "conservative scalar representation of a per-capability map, d6 included in INSERT/UPSERT, "
     "saved dict, scores dict passed to apply_score_update_enforcement(), get_dimension_history(), "
     "get_trends_and_volatility(), and the dimension-history API response. "
     "_DIM_COL and _VALID_DIMENSIONS in dimension_policy.py updated to include d6/capability_trust. "
     "[Medium] policy_set_version versioning fixed: PUT endpoint now queries "
     "SELECT COALESCE(MAX(policy_set_version), 0) + 1 before inserting, so each policy "
     "replacement atomically increments the version. Response now includes policy_set_version. "
     "[Medium] Per-candidate audit logging added to enforce_plan_dimension_policy_gate(): "
     "each candidate's pass/fail decision (with violated/passed policies or skip reason) "
     "is recorded in _candidate_audit and included in the corrections entry so run events "
     "carry full routing decision provenance. "
     "Files: agentid-pro/dimension_policy.py, agentid-pro/dimension_snapshots.py, "
     "agentid-pro/group_orchestrator.py."),
    ("v1.8.0", "2026-05-17",
     "Phase 1.1 — Dimensional Trust Policy Engine. "
     "NEW MODULE dimension_policy.py: unified predicate engine that applies per-dimension trust "
     "thresholds at two lifecycle points. "
     "dispatch trigger: evaluated at task dispatch time inside group_orchestrator — agents whose "
     "dimension scores violate an owner policy are replaced with the best alternative via "
     "enforce_plan_dimension_policy_gate(), called immediately after _enforce_plan_trust_gate(). "
     "score_update trigger: evaluated after every dimension_snapshots upsert — violations insert "
     "a row in agent_capability_restrictions; recovery increments hysteresis_count and lifts "
     "the restriction once count reaches policy.hysteresis (default 3), preventing flapping. "
     "Both triggers share one policy table, schema, and evaluate_policies() predicate function. "
     "DB tables: dimension_policies (owner, dimension d1-d6/composite, operator lt/gt/lte/gte, "
     "threshold 0-100, applies_to capability or *, trigger jsonb, hysteresis, label, versioned), "
     "agent_capability_restrictions (did, owner, capability, policy_id FK, triggered_dim, "
     "triggered_value, threshold, restricted_at, lifted_at, hysteresis_count). "
     "API endpoints: PUT /pro/dimension-policies (atomic replace, old versions deactivated), "
     "GET /pro/dimension-policies, DELETE /pro/dimension-policies, "
     "POST /pro/dimension-policies/simulate (dry-run: submit agent DIDs + scores, returns "
     "selected/rejected + exact predicate trace, no execution), "
     "GET /pro/agents/{did}/effective-capabilities (granted vs. active after restrictions), "
     "GET /pro/agents/{did}/restriction-history (full impose/lift history with hysteresis). "
     "Hooks: dimension_snapshots.take_snapshot_for_did() now calls apply_score_update_enforcement() "
     "after each upsert (fail-open). group_orchestrator.start_group_run() calls "
     "enforce_plan_dimension_policy_gate() after _enforce_plan_trust_gate() (fail-open). "
     "server_pro.py: imports router + init, registers router, calls init_dimension_policy_schema() "
     "after init_dimension_snapshots_schema(). "
     "Tests: tests/test_dimension_policy.py — 40 tests covering predicate evaluation, "
     "dimension normalisation, policy validation, dispatch gate (replacement logic, fallback), "
     "score-update enforcement (insert, hysteresis increment, lift). Full suite: 89/89 passed."),
    ("v1.7.2", "2026-05-17",
...
def build_tracker():
    ...
    built_sections = [
        ...
        ("v1.9.x", "Phase 1.2 — Causal Audit Graph (round-8 complete, 2026-05-17)", GREEN, [
            ("parent_event_id + causal_weight on group_run_events",
             "Self-referential FK (nullable) links child events to their parent. causal_weight FLOAT "
             "for ranking contributing causes. Schema init in group_orchestrator.py. "
             "New columns: parent_event_id BIGINT REFERENCES group_run_events(id), causal_weight FLOAT. "
             "_log_event() gains keyword-only parent_event_id + causal_weight params; validates parent "
             "using caller's conn (avoids cross-connection visibility issues for same-transaction parents). "
             "Full correction chain: run_created → orchestrator_plan_corrected → "
             "orchestrator_plan_dim_policy_corrected → orchestrator_plan. "
             "Approval chain: orchestrator_plan → approval_requested → approval_granted → agent_assigned. "
             "_plan_assign_ids dict captures RETURNING id from plan INSERTs so policy-denied and "
             "allow-path UPDATEs use WHERE id=%s (safe for duplicate-agent plans). "
             "Fallback denied UPDATE uses WHERE id=(SELECT id ... ORDER BY assigned_at LIMIT 1) "
             "— valid Postgres subquery replaces the bare LIMIT 1 that was invalid.",
             "agentid-pro/group_orchestrator.py  schema + _log_event + correction/approval chain\n"
             "agentid-pro/causal_graph.py  (new module)",
             "done"),
            ("causal_graph.py — GET /causal-graph + /causal-graph/trace",
             "GET /pro/groups/runs/{run_id}/causal-graph returns {nodes, edges} DAG. "
             "Each node carries trust_snapshot (trust_score + dimension_scores). "
             "Batch trust enrichment fills nodes lacking event-time scores via latest snapshot. "
             "Each edge carries weight + edge_type derived from 13-entry _EDGE_TYPE_MAP "
             "(correction, policy_correction, planning, approval_pending, approval, dispatch, "
             "execution, rejection, quality_fail, contribution, finalization, completion, failure; "
             "default: sequence). "
             "GET /pro/groups/runs/{run_id}/causal-graph/trace?event_id=X walks ancestor chain "
             "via recursive CTE (max_depth=50); same batch trust enrichment as full graph. "
             "Both endpoints owner-check the run (404 guard). "
             "Pre-Phase-1.2 runs return full event list with empty edges (audit log mode).",
             "agentid-pro/causal_graph.py  get_run_causal_graph() + trace_event_ancestors()\n"
             "agentid-pro/server_pro.py  init_causal_graph_schema() call",
             "done"),
            ("Round-8: approval dispatch capability parity (High fix)",
             "_dispatch_pending_assignments() now calls _load_capability_profile(_cap, owner, group_id) "
             "before the policy gate, mirroring normal dispatch. Resolved permission_level from profile "
             "overrides the stored value; assignment row UPDATE now includes required_capability and "
             "resolved_permission_level; agent_assigned event includes profile_resolved=True/False; "
             "_execute_subtask started with kwargs={'profile': _ap_profile}. "
             "Denied-path UPDATE in approval dispatch also updated to store required_capability and "
             "resolved_permission_level for schema parity with normal dispatch.",
             "agentid-pro/group_orchestrator.py  _dispatch_pending_assignments()",
             "done"),
            ("Test suite: 149/149 (27 causal_graph + 122 orchestrator/xalgo/other)",
             "test_causal_graph.py: 27 tests — edge classification (13 types + sequence fallback), "
             "trust snapshot extraction, batch enrichment for both full-graph and trace endpoints. "
             "test_group_orchestrator_xalgo.py: 5 new round-8 tests — "
             "test_dispatch_pending_logs_profile_resolved_in_agent_assigned, "
             "test_dispatch_pending_profile_kwarg_passed_to_execute_subtask, "
             "test_normal_dispatch_denied_fallback_uses_subquery_not_limit (source inspection). "
             "All 149 tests pass.",
             "agentid-pro/tests/test_causal_graph.py\n"
             "agentid-pro/tests/test_group_orchestrator_xalgo.py",
        ...
        ("v1.8.x", "Phase 1.1 — Dimensional Trust Policy Engine (round-4 complete, 2026-05-17)", GREEN, [
            ("dimension_policy.py — predicate engine (new module)",
             "Unified predicate engine applying dimension-level thresholds at dispatch + score-update. "
             "dispatch trigger: enforce_plan_dimension_policy_gate() called in _orchestrate() after "
             "_enforce_plan_trust_gate(); agents violating owner policies are replaced with best "
             "compliant alternative or blocked (assignment dropped if no compliant agent exists). "
             "score_update trigger: applied after every dimension_snapshots upsert; violations insert "
             "agent_capability_restrictions rows; recovery lifts restriction after hysteresis_count "
             "reaches policy.hysteresis (default 3). Both triggers share one predicate engine + table.",
             "agentid-pro/dimension_policy.py  (new)\n"
             "agentid-pro/group_orchestrator.py  enforce_plan_dimension_policy_gate() call",
             "done"),
            ("DB schema: dimension_policies + agent_capability_restrictions",
             "dimension_policies: owner, dimension (d1-d6/composite), operator (lt/gt/lte/gte), "
             "threshold 0-100, applies_to (capability or *), trigger jsonb, hysteresis, label, versioned. "
             "agent_capability_restrictions: did, owner, capability, policy_id FK, triggered_dim, "
             "triggered_value, threshold, restricted_at, lifted_at, hysteresis_count. "
             "D6 (capability_trust) added to dimension_snapshots and all dimension-related APIs.",
             "agentid-pro/dimension_policy.py  init_dimension_policy_schema()\n"
             "agentid-pro/dimension_snapshots.py  d6 column + take_snapshot_for_did()",
             "done"),
            ("API endpoints",
             "PUT /pro/dimension-policies — atomic replace (old versions deactivated), returns new version. "
             "GET /pro/dimension-policies — list active policies with policy_set_version. "
             "GET /pro/agents/{did}/effective-capabilities — granted vs. currently restricted. "
             "POST /pro/dimension-policies/simulate — dry-run routing decision (no execution). "
             "GET /pro/agents/{did}/dimension-history — per-dimension time series.",
             "agentid-pro/dimension_policy.py  router\n"
             "agentid-pro/dimension_snapshots.py  router",
             "done"),
            ("Round-4 fixes: blocked assignment lifecycle closed",
             "'denied' added to terminal status filter in _get_pending_assignments(), watchdog "
             "force-fail UPDATE, and _all_assignments_done() COUNT — partial-block runs now synthesize. "
             "Partial-block INSERT preserves subtask field. All-block failed run writes completed_at. "
             "has_active_restriction() wired into _run_policy_gate() — enforcement loop fully closed. "
             "40/40 dimension policy tests + 101/101 full suite passing.",
             "agentid-pro/dimension_policy.py, agentid-pro/group_orchestrator.py\n"
             "agentid-pro/tests/test_dimension_policy.py  (40 tests)",
             "done"),
        ]),
