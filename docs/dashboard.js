...
// ── AGENT TASKS ──────────────────────────────────────────────────────────────
let _tasksAgents = [];  // populated from /agents?mine=true
let _tasksPage   = 1;

function _taskStatusBadge(s) {
  const colors = {
    pending:'#6b7280', accepted:'#3b82f6', running:'#8b5cf6',
    completed:'#16a34a', failed:'#dc2626', rejected:'#dc2626',
    expired:'#d97706', cancelled:'#6b7280',
  };
  return `<span style="display:inline-block;padding:0.15rem 0.55rem;border-radius:999px;font-size:0.72rem;font-weight:600;background:${colors[s]||'#6b7280'}22;color:${colors[s]||'#6b7280'};">${s}</span>`;
}

async function _tasksLoadAgents() {
  try {
    const data = await apiFetch('/agents?mine=true&limit=100');
    _tasksAgents = Array.isArray(data) ? data : (data.agents || []);
    const sel = document.getElementById('tasks-from-did');
    if (!sel) return;
    if (!_tasksAgents.length) {
      sel.innerHTML = '<option value="">— no agents registered yet —</option>';
      return;
    }
    sel.innerHTML = _tasksAgents.map(a =>
      `<option value="${esc(a.did)}">${esc(a.name || a.did.slice(-20))}</option>`
    ).join('');
  } catch (_) {}
}

async function _tasksSend() {
  const fromDid    = document.getElementById('tasks-from-did').value;
  const toDid      = document.getElementById('tasks-to-did').value.trim();
  const capability = document.getElementById('tasks-capability').value.trim();
  const title      = document.getElementById('tasks-title').value.trim();
  const rawInputs  = document.getElementById('tasks-inputs').value.trim();
  const statusEl   = document.getElementById('tasks-send-status');
  const btn        = document.getElementById('tasks-send-btn');

  if (!fromDid)    { statusEl.textContent = '⚠ Select a From agent'; return; }
  if (!toDid)      { statusEl.textContent = '⚠ Enter a To Agent DID'; return; }
  if (!capability) { statusEl.textContent = '⚠ Enter a capability'; return; }
  ...
  try {
    const task = await apiFetch(`/agents/${encodeURIComponent(toDid)}/tasks`, {
      method: 'POST',
      body: JSON.stringify({ from_did: fromDid, capability, title: title || undefined, inputs, timeout_seconds: 300 }),
    });
    statusEl.textContent = `✓ Task sent — ID ${task.task_id.slice(0,8)}…  status: ${task.status}`;
    document.getElementById('tasks-to-did').value = '';
    document.getElementById('tasks-capability').value = '';
    document.getElementById('tasks-title').value = '';
    document.getElementById('tasks-inputs').value = '';
    _tasksLoadList(1);
  } catch (e) {
...
async function _tasksLoadList(page = 1) {
  _tasksPage = page;
  const wrap    = document.getElementById('tasks-list-wrap');
  const role    = document.getElementById('tasks-role-filter')?.value || '';
  const status  = document.getElementById('tasks-status-filter')?.value || '';
  if (!wrap || !_tasksAgents.length) {
    if (wrap) wrap.innerHTML = '<div style="padding:1.5rem 1.25rem;color:var(--muted);font-size:0.84rem;">Register an agent first to see tasks.</div>';
    return;
  }
  wrap.innerHTML = '<div class="loading" style="padding:1.5rem;"><div class="spinner"></div> Loading…</div>';
  const did = _tasksAgents[0].did;
  const params = new URLSearchParams({ limit: 20, offset: (page - 1) * 20 });
  if (role)   params.set('role', role);
  if (status) params.set('status', status);
  try {
    const data  = await apiFetch(`/agents/${encodeURIComponent(did)}/tasks?${params}`);
    const tasks = data.tasks || data || [];
    if (!tasks.length) {
      wrap.innerHTML = '<div style="padding:1.5rem 1.25rem;color:var(--muted);font-size:0.84rem;">No tasks yet. Send one above.</div>';
      return;
    }
    wrap.innerHTML = `
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="background:var(--surface2);">
          <th style="text-align:left;padding:0.5rem 1rem;font-size:0.72rem;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.04em;">ID</th>
          <th style="text-align:left;padding:0.5rem 0.75rem;font-size:0.72rem;font-weight:700;color:var(--muted);text-transform:uppercase;">Capability</th>
          <th style="text-align:left;padding:0.5rem 0.75rem;font-size:0.72rem;font-weight:700;color:var(--muted);text-transform:uppercase;">To / From</th>
          <th style="text-align:left;padding:0.5rem 0.75rem;font-size:0.72rem;font-weight:700;color:var(--muted);text-transform:uppercase;">Status</th>
          <th style="text-align:left;padding:0.5rem 0.75rem;font-size:0.72rem;font-weight:700;color:var(--muted);text-transform:uppercase;">Created</th>
          <th></th>
        </tr></thead>
        <tbody>` +
      tasks.map(t => `
        <tr style="border-top:1px solid var(--border);">
          <td style="padding:0.6rem 1rem;font-family:'JetBrains Mono',monospace;font-size:0.72rem;color:var(--muted);" title="${esc(t.task_id)}">${esc(t.task_id.slice(0,8))}…</td>
          <td style="padding:0.6rem 0.75rem;font-size:0.82rem;font-weight:600;">${esc(t.capability)}</td>
          <td style="padding:0.6rem 0.75rem;font-family:'JetBrains Mono',monospace;font-size:0.7rem;color:var(--muted);" title="${esc(t.to_did)}">${esc(t.to_did.slice(-18))}</td>
          <td style="padding:0.6rem 0.75rem;">${_taskStatusBadge(t.status)}</td>
          <td style="padding:0.6rem 0.75rem;font-size:0.75rem;color:var(--muted);white-space:nowrap;">${_fmtRelative(t.created_at)}</td>
          <td style="padding:0.6rem 0.75rem;display:flex;gap:0.4rem;">
            <button class="btn btn-outline" style="font-size:0.75rem;padding:0.2rem 0.6rem;"
              onclick="_tasksViewTask('${esc(t.task_id)}','${esc(t.to_did)}')">View</button>
            ${t.status === 'pending' ? `<button class="btn btn-outline" style="font-size:0.75rem;padding:0.2rem 0.6rem;color:#dc2626;border-color:#dc2626;"
              onclick="_tasksCancelTask('${esc(t.task_id)}','${esc(t.to_did)}')">Cancel</button>` : ''}
          </td>
        </tr>`).join('') + `</tbody></table>`;
  } catch (e) {
    wrap.innerHTML = `<div style="padding:1.5rem 1.25rem;color:#dc2626;font-size:0.84rem;">${esc(String(e))}</div>`;
  }
}
...
async function _tasksViewTask(taskId, toDid) {
  try {
    const t = await apiFetch(`/agents/${encodeURIComponent(toDid)}/tasks/${encodeURIComponent(taskId)}`);
    const output = t.output ? JSON.stringify(t.output, null, 2) : '(none yet)';
    const inputs = t.inputs ? JSON.stringify(t.inputs, null, 2) : '(none)';
    // Reuse the existing confirm modal as a read-only detail view
    document.getElementById('modal-title').textContent = `Task · ${taskId.slice(0,8)}…`;
    document.getElementById('modal-body').innerHTML = `
      <div style="display:grid;gap:0.4rem;font-size:0.82rem;">
        <div><strong>Status:</strong> ${_taskStatusBadge(t.status)}</div>
        <div><strong>Capability:</strong> ${esc(t.capability)}</div>
        ${t.title ? `<div><strong>Title:</strong> ${esc(t.title)}</div>` : ''}
        <div style="margin-top:0.5rem;"><strong>Inputs:</strong><pre style="margin:0.25rem 0 0;font-size:0.75rem;background:var(--surface2);padding:0.5rem;border-radius:6px;overflow:auto;max-height:120px;">${esc(inputs)}</pre></div>
        <div style="margin-top:0.4rem;"><strong>Output:</strong><pre style="margin:0.25rem 0 0;font-size:0.75rem;background:var(--surface2);padding:0.5rem;border-radius:6px;overflow:auto;max-height:120px;">${esc(output)}</pre></div>
      </div>`;
  ...
  } catch (e) { alert('Could not load task: ' + e); }
}

async function _tasksCancelTask(taskId, toDid) {
  if (!confirm('Cancel this task? The receiving agent will not execute it.')) return;
  try {
    await apiFetch(`/agents/${encodeURIComponent(toDid)}/tasks/${encodeURIComponent(taskId)}`, { method: 'DELETE' });
    _tasksLoadList(_tasksPage);
  } catch (e) { alert('Error: ' + e); }
}

function _initTasks() {
  _tasksLoadAgents().then(() => _tasksLoadList(1));

  document.getElementById('tasks-send-btn')?.addEventListener('click', _tasksSend);
  document.getElementById('tasks-refresh-btn')?.addEventListener('click', () => _tasksLoadList(1));
  document.getElementById('tasks-role-filter')?.addEventListener('change', () => _tasksLoadList(1));
  document.getElementById('tasks-status-filter')?.addEventListener('change', () => _tasksLoadList(1));

  // Discover toggle
  document.getElementById('tasks-discover-toggle')?.addEventListener('click', () => {
    const p = document.getElementById('tasks-discover-panel');
    if (p) p.style.display = p.style.display === 'none' ? 'block' : 'none';
  });
  document.getElementById('tasks-discover-search')?.addEventListener('click', async () => {
    const q = document.getElementById('tasks-discover-q')?.value.trim() || '';
    const res = document.getElementById('tasks-discover-results');
    if (!res) return;
    res.innerHTML = '<div class="spinner" style="width:16px;height:16px;"></div>';
    try {
      const data   = await apiFetch(`/agents?capability=${encodeURIComponent(q)}&limit=10`);
      const agents = Array.isArray(data) ? data : (data.agents || []);
      if (!agents.length) { res.textContent = 'No agents found.'; return; }
      res.innerHTML = agents.map(a => `
        <div style="display:flex;align-items:center;gap:0.6rem;padding:0.4rem 0;border-top:1px solid var(--border);">
          <div style="flex:1;min-width:0;">
            <div style="font-weight:600;font-size:0.82rem;">${esc(a.name||'—')}</div>
            <div style="font-family:'JetBrains Mono',monospace;font-size:0.7rem;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(a.did)}</div>
          </div>
          <button class="btn btn-outline" style="font-size:0.75rem;padding:0.2rem 0.6rem;white-space:nowrap;"
            onclick="document.getElementById('tasks-to-did').value='${esc(a.did)}';document.getElementById('tasks-discover-panel').style.display='none';">
            Use
          </button>
        </div>`).join('');
    } catch (e) { res.textContent = 'Error: ' + e; }
  });
  document.getElementById('tasks-discover-q')?.addEventListener('keydown', e => {
    if (e.key === 'Enter') document.getElementById('tasks-discover-search')?.click();
  });
}

// ── SIDEBAR NAV ──────────────────────────────────────────────────────────────
const _NAV_SECTIONS = ['overview','agents','analytics','network','audit','signing','playground','tasks'];
